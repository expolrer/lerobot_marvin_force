...
def resolve_delta_timestamps(
    ...
    for key in ds_meta.features:
        feature_delta_resolver = getattr(cfg, "delta_indices_for_feature", None)
        if callable(feature_delta_resolver):
            feature_delta_indices = feature_delta_resolver(key)
            if feature_delta_indices is not None:
                delta_timestamps[key] = [i / ds_meta.fps for i in feature_delta_indices]
                continue
