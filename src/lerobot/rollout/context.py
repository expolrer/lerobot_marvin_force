...
def build_rollout_context(
    ...
    # Position, estimated external joint force, and feedback joint torque are
    # routed to the policy. Velocity and Cartesian wrench channels remain
    # excluded. The policy config still selects exactly one force feature, so
    # joint_force and joint_torque are never treated as interchangeable.
    ...
    observation_features_hw = {
        ...
        or (
            v is float
            and (k.endswith(".pos") or k.endswith(".force") or k.endswith(".torque"))
        )
    }
